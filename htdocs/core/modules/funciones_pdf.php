	...
	*/
				...
				|| (($tipo == 1) ||($tipo ==3))) && (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'CHQ'))
				{
					...
					if (! empty($conf->global->FACTURE_CHQ_NUMBER))
					{
						...
						if ($conf->global->FACTURE_CHQ_NUMBER == -1)
						{
							$pdf->SetXY($pagina->marge_gauche, $posy);
							$pdf->SetFont('', 'B', $default_font_size - $diffsizetitle);
							$pdf->MultiCell(100, 3, $outputlangs->transnoentities('PaymentByChequeOrderedTo',$pagina->emetteur->name), 0, 'L', 0);
							$posy=$pdf->GetY()+1;
							...
							if (empty($conf->global->MAIN_PDF_HIDE_CHQ_ADDRESS))
							{
								$pdf->SetXY($pagina->marge_gauche, $posy);
								$pdf->SetFont('', '', $default_font_size - $diffsizetitle);
								$pdf->MultiCell(100, 3, $outputlangs->convToOutputCharset($pagina->emetteur->getFullAddress()), 0, 'L', 0);
								$posy=$pdf->GetY()+2;
							}
						}
					}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='', &$pagina, $tipo)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->SetXY($pagina->page_largeur - $pagina->marge_droite - ($pdf->GetStringWidth($titre) + 3), $tab_top-4);
		...
		if ($tipo == 2){
			...
			// Accountancy piece
			if (empty($hidetop)) {
				$pdf->SetXY($pagina->posxpiece-1, $tab_top+1);
				$pdf->MultiCell($pagina->posxcomment-$pagina->posxpiece-1, 1, '', '', 'R');
			}		

		// Comments
			$pdf->line($pagina->posxcomment-1, $tab_top, $pagina->posxcomment-1, $tab_top + $tab_height);
			if (empty($hidetop)) {
				$pdf->SetXY($pagina->posxcomment-1, $tab_top+1);
				$pdf->MultiCell($pagina->posxdate-$pagina->posxcomment-1, 1, $outputlangs->transnoentities("Description"), '', 'L');
			}
		}else{
			$pdf->SetXY($pagina->posxdesc-1, $tab_top+1);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($pagina->posxtva-3, $tab_top+1);
				$pdf->MultiCell($pagina->posxup-$pagina->posxtva+3 ,2, $outputlangs->transnoentities("VAT"), '', 'C');
			}
		}
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($pagina->posxup-1, $tab_top+1);
			$pdf->MultiCell($pagina->posxqty-$pagina->posxup-1, 2, $outputlangs->transnoentities("PriceUHT"), '', 'C');
		}

		$pdf->line($pagina->posxqty-1, $tab_top, $pagina->posxqty-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($pagina->posxqty-1, $tab_top+1);

			if(($tipo ==1) && ($pagina->situationinvoice))
			{
				$pdf->MultiCell($pagina->posxprogress-$pagina->posxqty-1, 2, $outputlangs->transnoentities("Qty"), '', 'C');
			}
			elseif($tipo ==2){
			$pdf->MultiCell($pagina->postotalttc-$pagina->posxqty - 1, 2, $outputlangs->transnoentities("Qty"), '', 'C');
			}
			
			elseif($conf->global->PRODUCT_USE_UNITS)
			{
				$pdf->MultiCell($pagina->posxunit-$pagina->posxqty-1, 2, $outputlangs->transnoentities("Qty"), '', 'C');
			
			}else{
				$pdf->MultiCell($pagina->posxdiscount-$pagina->posxqty-1, 2, $outputlangs->transnoentities("Qty"), '',' C');
			}
		}
		...
		if ($tipo ==2){
			...
			if (empty($hidetop)) {
				$pdf->SetXY($pagina->postotalttc-1, $tab_top+1);
				$pdf->MultiCell($pagina->page_largeur-$pagina->marge_droite-$pagina->postotalttc, 2, $outputlangs->transnoentities("TotalTTC"), '', 'C');
		}
		}else {
		if (($tipo ==1) && ($pagina->situationinvoice)) {
			...
			if (empty($hidetop)) {
				...
				$pdf->SetXY($pagina->posxprogress, $tab_top+1);

				if($conf->global->PRODUCT_USE_UNITS)
				{
					$pdf->MultiCell($pagina->posxunit-$pagina->posxprogress, 2, $outputlangs->transnoentities("Progress"), '', 'C');
				}
				elseif ($pagina->atleastonediscount)
				{
					$pdf->MultiCell($pagina->posxdiscount-$pagina->posxprogress, 2, $outputlangs->transnoentities("Progress"), '', 'C');
				}
				else
				{
					$pdf->MultiCell($pagina->postotalht-$pagina->posxprogress, 2, $outputlangs->transnoentities("Progress"), '', 'C');
				}
			}
		}
		...
		if($conf->global->PRODUCT_USE_UNITS) {
			...
			if (empty($hidetop)) {
				$pdf->SetXY($pagina->posxunit - 1, $tab_top + 1);
				$pdf->MultiCell($pagina->posxdiscount - $pagina->posxunit - 1, 2, $outputlangs->transnoentities("Unit"), '', 'C');
			}
		}
		...
		if (empty($hidetop))
		{
			...
			{
				$pdf->SetXY($pagina->posxdiscount-1, $tab_top+1);
				$pdf->MultiCell($pagina->postotalht-$pagina->posxdiscount+1, 2, $outputlangs->transnoentities("ReductionShort"), '', 'C');
			}
		}
		...
		if (($tipo ==1) && ($pagina->situationinvoice))
		{
			...
			if (empty($hidetop))
			{
				$pdf->SetXY($pagina->postotalht-19, $tab_top+1);
		...
		if (empty($hidetop))
		{
			$pdf->SetXY($pagina->postotalht-1, $tab_top+1);
...
//
		switch ($tipo){
			case 0:{
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell($w, 3, $outputlangs->transnoentities("DateEndPropal")." : " . dol_print_date($object->fin_validite,"day",false,$outputlangs,true), '', 'R');
			breal;}
			case 1:{
			...
			case 3:{				
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell($w, 3, $outputlangs->transnoentities("OrderDate")." : " . dol_print_date($object->date,"%d %b %Y",false,$outputlangs,true), '', 'R');
				}
		}
		if (($tipo ==0) || ($tipo ==1)) 
		{
...
//
		...
		// Date start period
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($pagina->page_largeur-$pagina->marge_droite-$posx, 3, $outputlangs->transnoentities("DateStart")." : " . ($object->date_debut>0?dol_print_date($object->date_debut,"day",false,$outputlangs):''), '', 'R');

			// Date end period
			$posy+=5;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($pagina->page_largeur-$pagina->marge_droite-$posx, 3, $outputlangs->transnoentities("DateEnd")." : " . ($object->date_fin>0?dol_print_date($object->date_fin,"day",false,$outputlangs):''), '', 'R');
			...
			$pdf->MultiCell($pagina->page_largeur-$pagina->marge_droite-$posx, 3, $object->getLibStatut(0), '', 'R');
			}
		...
		if ($showaddress)
		{
			...
			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox-2, 4, $outputlangs->convToOutputCharset($pagina->emetteur->name), 0, 'L');
			$posy=$pdf->getY();
