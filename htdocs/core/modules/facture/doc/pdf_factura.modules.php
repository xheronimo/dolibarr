 ...
 */
class pdf_factura extends ModelePDFFactures
{
     /**
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$top_shift = _pagehead($pdf, $object, 1, $outputlangs, $this, 1);
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					if (isset($imglinesize['width']) && isset($imglinesize['height']) && ($curY + $imglinesize['height']) > ($this->page_hauteur-($heightforfooter+$heightforfreetext+$heightforinfotot)))	// If photo too high, we moved completely on new page
					{
						$pdf->AddPage('', '', true);
						if (! empty($tplidx)) $pdf->useTemplate($tplidx);
						if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) _pagehead($pdf, $object, 0, $outputlangs, $this, 1);
					...
					while ($pagenb < $pageposafter)
					{
						...
						if ($pagenb == 1)
						{
							_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforfooter, 0, $outputlangs, 0, 1, $object->multicurrency_code, $this, 1);
						}
						else
						{
							_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 1, 1, $object->multicurrency_code, $this, 1);
						}
						_pagefoot($pdf, $object, $outputlangs, 1, $this, 1);
						$pagenb++;
						$pdf->setPage($pagenb);
						$pdf->setPageOrientation('', 1, 0);	// The only function to edit the bottom margin of current page to set it.
						if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) _pagehead($pdf, $object, 0, $outputlangs, $this, 1);
				...
				if ($pagenb == 1)
				{
					_tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 0, 0, $object->multicurrency_code, $this, 1);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}
				else
				{
					_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code, $this, 1);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}

				// Affiche zone infos
				$posy=_tableau_info($pdf, $object, $bottomlasttab, $outputlangs, $this, 1);

				// Affiche zone totaux
				$posy=_tableau_tot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs, $this, 1);
				...
				// Pied de page
				_pagefoot($pdf, $object, $outputlangs, 0 , $this, 1);
	 ...
	 */
	private function _tableau_versements(&$pdf, $object, $posy, $outputlangs, $heightforfooter)
		{
        // phpcs:enable
		global $conf;
